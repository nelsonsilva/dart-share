...
class DocEvent extends event.Event {
  ...
  var snapshot;
  var oldSnapshot;
  DocEvent({this.docName: null, this.data: null, this.snapshot: null, this.oldSnapshot: null}) : super(ID);
}
...
class Model implements event.Emitter<ModelEvents> {
    ...
    return new SyncQueue( (opData, callback) {
      ...
      getOps(docName, opData.version, doc.version).then((ops) {
        ...
        writeOp.then((_) {
          ...
          on.applyOp.dispatch(new DocEvent(docName: docName, data: opData, snapshot: snapshot, oldSnapshot: oldSnapshot));
          // TODO(nelsonsilva) - figure out why this crashes
          doc.on.op.dispatch(new DocEvent(docName: docName, data: opData, snapshot: snapshot, oldSnapshot: oldSnapshot));
  ...
  DocEntry add(docName, [DBDocEntry data, int committedVersion, ops]) {
    ...
    //} else {
      ...
      on.add.dispatch(new DocEvent(docName: docName, data: data));
  ...
  Future<DocEntry> load(docName) {
    ...
    return db.getSnapshot(docName).chain((DBDocEntry data) {
      ...
      return _getOpsInternal(docName, OT[data.type], data.version).transform( (ops) {
        ...
        on.load.dispatch( new DocEvent(docName: docName, data: data));
  ...
  Future<Doc> create(String docName, OTType type, DocMeta meta) {
    ...
    return db.create(docName, data).transform( (dbDoc) {
      ...
      //data.type = type.name;
      on.create.dispatch(new DocEvent(docName: docName, data: dbDoc));
  ...
  Future delete(docName) {
    ...
    deleteTask.then((_) {
      on.delete.dispatch(new DocEvent(docName: docName));
    });
    return deleteTask;
  }
