class TestText {
  static void run(){
    ...
    group('transform', (){
      ...
      test('insert', () {
        expect(
          [Text.Op().I('x', 10), Text.Op().I('a', 1)] == Text.Op().I('x',9).transform(Text.Op().I('a',1))
        );
        
        expect(
          [Text.Op().I('x', 10), Text.Op().I('a', 11)] ==  Text.Op().I('x', 10).transform(Text.Op().I('a',10))
        );
        
        // test.deepEqual [[{i:'x', p:10}], [{d:'a', p:9}]], type.transformX [{i:'x', p:11}], [{d:'a', p:9}]
        expect(
          [Text.Op().I('x', 10), Text.Op().D('a', 9)] == Text.Op().I('x', 11).transform(Text.Op().D('a',9))
        );
      ...
      test('delete', () {
        expect(
          [Text.Op().D('abc', 8), Text.Op().D('xy', 4)] == Text.Op().D('abc', 10).transform(Text.Op().D('xy', 4))
        );
        expect(
          [Text.Op().D('ac', 10), Text.Op()] == Text.Op().D('abc', 10).transform(Text.Op().D('b', 11))
        );
        expect(
          [Text.Op(), Text.Op().D('ac', 10)] == Text.Op().D('b', 11).transform(Text.Op().D('abc', 10))
        );
        expect(
          [Text.Op().D('a', 10), Text.Op()] == Text.Op().D('abc', 10).transform(Text.Op().D('bc', 11))
        );
        expect(
          [Text.Op().D('c', 10), Text.Op()] == Text.Op().D('abc', 10).transform(Text.Op().D('ab', 10))
        );
        expect(
          [Text.Op().D('a', 10), Text.Op().D('d', 10)] == Text.Op().D('abc', 10).transform(Text.Op().D('bcd', 11))
        );
        expect(
          [Text.Op().D('d', 10), Text.Op().D('a', 10)] == Text.Op().D('bcd', 11).transform(Text.Op().D('abc', 10))
        );
        expect(
          [Text.Op().D('abc', 10), Text.Op().D('xy', 10)] == Text.Op().D('abc', 10).transform(Text.Op().D('xy', 13))
        );
      });
    });
  }
